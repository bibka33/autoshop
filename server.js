...
// ========== ИНИЦИАЛИЗАЦИЯ ТОВАРОВ (ПОЛНАЯ ВЕРСИЯ) ==========
function initProducts() {
  ...
  if (count.count === 0) {
    ...
    console.log('📦 Товары уже есть в БД');
  }
}
...
app.post('/api/support', (req, res) => {
  ...
  console.log('Заявка в поддержку:', {name, contact, message});
  res.json({
    success: true,
    status: 'offline',
...
app.listen(PORT, () => {
  ...
  console.log(`📊 Отчёты: http://localhost:${PORT}/reports.html`);
});